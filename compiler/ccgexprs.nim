...
proc expr(p: BProc, n: PNode, d: var TLoc) =
  ...
  # M5 Lever A: per-expression sourcemap anchor.
  #
  # `expr()` is the single dispatch through which every visited
  # expression node passes. Attach a zero-width sourcemap annotation
  # here so every sub-expression contributes a segment, not just the
  # first byte of each statement that `genStmts` / `genLineDir` cover.
  #
  # The annotation goes into `cpsStmts` of the current block — the
  # same section where any C bytes generated by the dispatch below
  # will land. The point annotation is zero-width, so it just marks
  # "the next emitted byte at this offset belongs to <info>".
  #
  # `recordAt` itself bridges expanded.nim positions back to the user
  # source via `bridgeExpansionInfo` (M5 Lever B), so this point also
  # covers macro/template expansion bodies whose `info` was rewritten
  # by the renderer.
  if optSourcemap in p.config.globalOptions and
      n.info.fileIndex != InvalidFileIdx and
      n.info.safeLineNm > 0:
    recordAt(procSection(p, cpsStmts), n.info)
