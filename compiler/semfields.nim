...
proc semForObjectFields(c: TFieldsCtx, typ, forLoop, father: PNode) =
  ...
  of nkRecCase:
    ...
    for i in 1..<typ.len:
      ...
      # CTFS-M-VariantStep: copyTree preserves the source position of the
      # `of <X>:` (or `else:`) clause inside the TYPE DEFINITION. That info
      # leaks into runtime opcodes (opcBranch) generated by `genCase`,
      # because vmgen tags the opcode with the `nkOfBranch.info`. A debugger
      # user stepping through the synthesized `case` (e.g. inside the generic
      # `$` for a variant object) would otherwise see the program jump
      # backward into a type definition. Retag the cloned branch — and its
      # matching literal/range/Range children — with the for-loop's info so
      # the opcodes carry the synthesized site instead.
      branch.info = forLoop.info
      for j in 0 ..< branch.len - 1:
        branch[j].info = forLoop.info
